...
def get_character_image_path(god_id: int):
    """
    god_id: 0..11
    あなたの命名（CHAR_p1.png ... CHAR_p12.png）を優先して探し、
    既存互換で character_01.png 形式も探す。
    """
    n = int(god_id) + 1
    ...
    candidates_dirs = [
        ...
        Path(r"assets\images\characters"),
    ]

    filename_candidates = [
        f"CHAR_p{n}.png",          # ←あなたの現行
        f"CHAR_p{n:02d}.png",      # 念のため
        f"character_{n:02d}.png",  # 旧想定
        f"god_{n:02d}.png",        # 念のため
    ]
    ...
    # 最後の保険：番号を含むpngを拾う（例: *_p1.png など）
    for d in candidates_dirs:
        if d.exists():
            hits = list(d.glob(f"*{n}*.png"))
            if hits:
                return str(hits[0])
